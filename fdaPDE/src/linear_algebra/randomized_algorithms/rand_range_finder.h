...
//Randomized Range Finder
  template<typename MatrixType, IterationPolicy ItPolicy, StoppingPolicy StopPolicy> class RFIterations; // boh, sto RFIterations....

  template <typename MatrixType, IterationPolicy ItPolicy, StoppingPolicy StopPolicy> class RandomizedRangeFinder {
     private:
      // method parameters
      int block_sz_;
      ...
      int n_sing_vecs_ = 1;
      int maxIter_ = 1e4;   // some camel-case, other not..... ok

      // Storage of the decomposition
      DMatrix<double> X_, Y_;                   // range and corange (but they are stored again by copy in the RFIterations, and then you make another copy here??)
    ...
    RFIterations<MatrixType, ItPolicy, StopPolicy> InitRFIterations(const MatrixType& A) { // ok, but why?? try to mimics what done in regularized_svd, nut in the for loop below,
     ...
     public:
      ...
      RandomizedRangeFinder(const MatrixType& A, int block_sz, int seed = fdapde::random_seed) :
          block_sz_(block_sz), seed_(seed) {
          compute(A);
      }
      // computation
      RandomizedRangeFinder& compute(const MatrixType& A) {
	for (auto rf_it = this->InitRFIterations(A); !rf_it.stop(); ++rf_it) { } // what kind of contorto ragionamento c'è qui?
          return *this;
      }
      // setters

    // can we provide all these to the compute method? ok, it is default constructible, and then via compute you provide your parameters
	...
	if (svd_.cols() == -1) { svd_.compute(T_, Eigen::ComputeThinU | Eigen::ComputeThinV); } // svd_.cols() == -1, does this work? there is ever a case when this is true
          return svd_;
      }
      // freeing the space occupied by the storage of the decompositions
    
    void flush() { // ok, you flush essentially everything apart very little details...
          X_.resize(0, 0);
          Y_.resize(0, 0);
          T_.resize(0, 0);
...
public:
	...
	// this might be moved as functors in an internal namespace, and provided by the user
	// something like internals::reconstruction_accuracy
	// internals::singular_values_accuracy
  ...
  // this is destructor, why are you initializing something here? :O, ah you are changing the rf_ when this gets destroyed, bad design
    ~RFIterations(){
  ...
  // this extended version is something used only in the paper.... check if we really need this, and if this is better then keep this and discard the other
  
//ExtendedSubspaceIterations
template<typename MatrixType, StoppingPolicy StopPolicy>
class RFIterations<MatrixType,IterationPolicy::ExtendedSubspaceIterations,StopPolicy>{
private:
    MatrixType A_; //matrix to be decomposed (why are you performing a copy?)
    double norm_A_;
    //Storage of the decomposition
    DMatrix<double> X_, Y_; //range and corange
    DMatrix<double> T_; //core matrix
...
public:
  ...
  // ah ok, this is something to move outside? don't understand the rationality.... but here is the implementation of the RSI (extended)
    RFIterations& operator++(){
        if(index_%2 == 0){
            Y_= A_.transpose() * X_;
  ...
  // I've noticed this now... the IterationPolicy is something which is clearly specific of the randomized method...
  // why are you doing this?????

  // also, you are not searching the rank, but the range (the space having dimension rank)
...
private:
    ...
    int index_;
    //Pointer to the calling Range Finder
  RandomizedRangeFinder<MatrixType,IterationPolicy::BlockKrylovIterations,StopPolicy> *rf_; // also this.... again is tailored to the iteration policy
public:
    ...
    bool stop(){
      ...
      // is this equal to the other, or the stopping criteria are bounded to the type of iteration

      // no it is different, hence functor idea is useless, well you can provide this at constructor like flags, no need to have them as template
    ...
    // BCGS is the block classical grahm-schmidth
    // see Block Gram-Schmidt algorithms and their stability properties, Erin Carsona, Kathryn Lunda, Miroslav
    // Rozloznıkb, Stephen Thomasc

    // put it outside as a separate concept
...
public:
    ...
    RFIterations& operator++(){
	    ...
	    // if this is a BCGS could be factored outside
