...
int main() {
    ...
    for (const auto& r : ref) {
        ...
        auto grad_solution = [&](const Vec& p) {
            double t1, t2;
            auto u = mesh.invert_point(p, t1, t2, 5);  // called once
            return solution.phys_grad(u);             // returns full gradient
        };

        VectorField<M, M, Fun> df_appx;
        for (int i = 0; i < M; ++i) {
            df_appx(i, 0) = [=](const Vec& p) {
                const auto g = grad_solution(p);  // compute once
                return g(i);                      // extract component
            };
        }

        auto errorL2 = std::sqrt(integral(mesh, QGL2DP9)(err_physical * err_physical));
        auto errorH1 = std::sqrt( errorL2 * errorL2 + integral(mesh, QGL2DP9)(dot(df_appx - df_exact, df_appx - df_exact)));
