...
public:
    ...
    MatrixType hessian(const VectorType& p) const {
        ...
        for (int i = 0; i < M; ++i) {
            B[i]   = spline_basis_[i]->evaluate_basis(p(i));
            dB[i]  = spline_basis_[i]->evaluate_der_basis(p(i), 1);
            ddB[i] = spline_basis_[i]->evaluate_der_basis(p(i), 2);
        }
        ...
        for (int i = 0; i < M; ++i) {
            num_grad(i) = num / parts[i][index_[i] - minIdx_[i]] * dB[i][index_[i]];
        ...
        for (int i = 0; i < M; ++i) {
            for (int j = 0; j < M; ++j) {
                Scalar num_ij;
                if (i == j) {
                    num_ij = (num / parts[i][index_[i] - minIdx_[i]]) * ddB[i][index_[i]];
                } else {
                    num_ij = (num / (parts[i][index_[i] - minIdx_[i]] * parts[j][index_[j] - minIdx_[j]])) *
                             dB[i][index_[i]] * dB[j][index_[j]];
                }

                auto parts_d = parts;
                for (int k = 0; k < extents_[i]; ++k)
                    parts_d[i][k] = dB[i][minIdx_[i] + k];
                if (i == j) {
                    for (int k = 0; k < extents_[i]; ++k)
                        parts_d[i][k] = ddB[i][minIdx_[i] + k];
                } else {
                    for (int k = 0; k < extents_[j]; ++k)
                        parts_d[j][k] = dB[j][minIdx_[j] + k];
                }

                Scalar den_ij = multicontract<M>(weights_, parts_d);
                Scalar term1 = num_ij * den;
                Scalar term2 = num_grad(i) * den_grad(j);
                Scalar term3 = num_grad(j) * den_grad(i);
                Scalar term4 = num * den_ij;

                H(i, j) = (term1 * den - term2 - term3 - term4 + 2 * den_grad(i) * den_grad(j) * num) / (den * den * den);
            }
        }
