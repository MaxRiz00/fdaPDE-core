...
class Spline {
    ...
    double operator() (const std::array<double,M>& x) {
        ...
        for(int dim = 0; dim < M; dim++){
            value *= evaluate1D(knots_[dim], degree_[dim], index_[dim], x[dim]);
        }
        return value;
    }
...
int main() {
    ...
    std::array<std::vector<double>, 2> knots = {
        std::vector<double>{0,0,0,1,2,3,4,4,5,5,5},
        std::vector<double>{0,0,0,1,2,3,4,4,5,5,5}};
    ...
    auto f = Spline<2>(knots, degree, index);

    std::array<double,2> P = {0. , 0.5};
