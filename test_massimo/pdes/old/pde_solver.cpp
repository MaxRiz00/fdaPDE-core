...
int main(){
    ...
    //mesh.refine_knots({0,2});

    std::string save_path = "../sphere_refined/"; // or wherever you want
    export_mesh(mesh, save_path);
    ...
    //ScalarField<3, decltype([](const Eigen::Matrix<double, 3, 1>& p) { return p(0); })> u;
    ...
    // save in a file uh_reduced
    std::ofstream uh_reduced_file("../uh_reduced.txt");
    uh_reduced_file << std::fixed << std::setprecision(10) << Eigen::MatrixXd(uh_reduced) << std::endl;
    // save in a file uh_full
    std::ofstream uh_full_file("../uh_full.txt");
    uh_full_file << std::fixed << std::setprecision(10) << Eigen::MatrixXd(uh_full) << std::endl;
